...
class _MixScreenState extends State<MixScreen> {
  ...
  Future<void> _load() async {
    setState(() { _loading = true; _status = 'Carregando todos os esportes...'; });
    try {
      final result = await ApiService.fetchAllProps();
      final ranked = EdgeEvaluatorService.rankLocal(
          result.data.where(_jogoValido).toList());
      setState(() {
        _allProps = ranked;
  ...
  void _showEvaluation() {
    ...
    if (top.isEmpty) { _showError('Sem props disponíveis.'); return; }
            ...
            child: _filtered.isEmpty && !_loading
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      final p = _filtered[i];
                      final sport = p['sport'] as String? ?? '';
                      return PropCard(prop: p, sportBadge: sport.isNotEmpty ? sport : null);
                    },
                  ),
          ),
        ],
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Row(
            children: [
              const Text('Edge mín:', style: TextStyle(color: Color(0xFF888888), fontSize: 12)),
                  ...
                  child: GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                          color: selected ? const Color(0xFF00C853) : const Color(0xFF444466),
                        ),
                      ),
                      child: Text(
                        v == 0 ? 'Todos' : '${v.toInt()}%',
                        style: TextStyle(
                          color: selected ? const Color(0xFF00C853) : const Color(0xFF888888),
                          fontSize: 12,
                        ),
                      ),
                    ),
              ...
              GestureDetector(
                ...
                child: Row(
                  children: [
                    Icon(
                      _hideWarnings ? Icons.warning_amber : Icons.warning_amber_outlined,
                      color: _hideWarnings ? const Color(0xFFFF6D00) : const Color(0xFF666666),
                      size: 18,
                    ),
