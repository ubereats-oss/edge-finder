...
class _BaseballScreenState extends State<BaseballScreen>
  ...
  Future<void> _runQuick() async {
    ...
    try {
      await ApiService.post('mlb/update-odds');
      setState(() => _status = 'Buscando props...');
      await ApiService.post('mlb/update-props');
      setState(() => _status = 'Rodando modelos...');
      await ApiService.post('mlb/run-model');
      await ApiService.post('mlb/run-props-model');
  ...
  Future<void> _runFull() async {
    setState(() { _loading = true; _status = 'Atualizando scores históricos (~45 min)...'; });
    try {
      await ApiService.post('mlb/update-scores');
        ...
        child: Column(
          ...
          children: [
            ...
            _UpdateOption(
              ...
              subtitle: 'Scores históricos · ~45 minutos',
              ...
              children: [
                Column(
                  children: [
                    ...
                    Expanded(
                      child: _h2hResults.isEmpty && !_loading
                          ? const _EmptyState(
                              msg: 'Sem jogos disponíveis.\nAtualize para buscar.')
                          : ListView.builder(
                              padding:
                                  const EdgeInsets.symmetric(vertical: 12),
                              itemCount: _h2hResults.length,
                              itemBuilder: (_, i) =>
                                  MatchCard(match: _h2hResults[i]),
                            ),
                    ),
                  ],
                ),
