...
async function getMlbOdds() {
  ...
  for (let i = 0; i < API_KEYS.length; i++) {
    try {
      const url = `https://api.the-odds-api.com/v4/sports/baseball_mlb/odds/?apiKey=${API_KEYS[i]}&regions=eu&markets=h2h&oddsFormat=decimal`;
      ...
      response.data.forEach(match => {
        ...
        odds.push({ team1: t1.name, team2: t2.name, odds1: t1.price, odds2: t2.price, commence_time: match.commence_time, event_id: match.id });
      });
  ...
  console.error('Erro ao buscar odds MLB: todas as chaves esgotadas.', lastError?.response?.data || lastError?.message);
}
