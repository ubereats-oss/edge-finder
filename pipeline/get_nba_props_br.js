...
function parseBookmakerProps(data, event, playerTeam, allProps, pinnacleMatchups) {
...
async function getNbaProps() {
  ...
  try {
    pinnacleMatchups = await fetchPinnacleMatchups(487);
    console.log(`Pinnacle matchups NBA: ${pinnacleMatchups.length}`);
  } catch(e) {
  ...
  try {
    ...
    for (const event of events) {
      ...
      try {
        ...
        const found = parseBookmakerProps(data, event, playerTeam, allProps, pinnacleMatchups);
        if (!found) {
...
try {
  oddsApi = createOddsApiClient({ label: 'NBA BR props' });
} catch (e) {
